 ...
 * screen requires calibration, which is done by touching the screen edges.
 * The remote sensors can be given a 3 character name from the menu.
 *
...
typedef enum {SCREEN, MENU, CALIBRATE, KEYBOARD} view_t;

view_t view = SCREEN;
bool calibrated = false, dst = true, old_auto_led, is_day;
button_t b_dst = {.value = true}, b_auto_led = {.value = true};
button_t b_dark_mode, b_auto_mode = {.value = true};
int8_t tz = 1, new_tz = 1;
uint8_t old_ocr0b;
uint16_t fgcolor = ILI9341_WHITE, bgcolor = ILI9341_BLACK;

const char keys[] PROGMEM = "!@#$%^&*()1234567890qwertyuiopasdfghjkl\x1Ezxcvbnm \x1B";
...
static void updateScreen(void) {
	...
	for (uint8_t i = 0; i < SENSOR_COUNT; i++) {
		...
		if (remote[i].unit.result) {
			...
			drawScaledLeft(30,y,60,remote[i].temp);
			drawSymbol(9);
			ili9341_write('C');
...
// draw and handle touchscreen buttons
static button_t handleButton(uint16_t x, uint16_t y, const char *str, char c, button_t button) {
	...
	// invert color when button touched or has value
	if (button.all) ili9341_setTextColor(bgcolor,fgcolor);
...
static void drawMenu(void) {
	view = MENU;
	ili9341_fillScreen(bgcolor);
	ili9341_drawRect(0,0,320,240,ILI9341_GRAY);
	ili9341_setCursor(1,0);
	ili9341_setTextColor(fgcolor, ILI9341_GRAY);
...
static void drawCalibrate(void) {
	...
	ili9341_fillTriangle(0,0,20,0,0,20,fgcolor);
	ili9341_fillTriangle(319,219,299,239,319,239,fgcolor);
}

// initialize keyboard screen
...
static void updateMenu(void) {
	static button_t b_ok, b_cancel, b_tz_up, b_tz_down, b_led_up, b_led_down, b_names, b_calibrate;
	
	// draw time zone
	ili9341_setCursor(10,34);
	...
	// draw LED percentage
	ili9341_setCursor(10,76);
	ili9341_puts_p(PSTR("Backlight "));
	drawInt(map((uint16_t)OCR0B, 0, 255, 0, 100));
	ili9341_write('%');
	ili9341_clearTextArea(119);
	// draw and handle buttons
	ili9341_setCursor(10,118);
	ili9341_puts_p(PSTR("Dark mode"));
	ili9341_setTextSize(2);
	b_tz_up = handleButton(120,26,PSTR(""),'+', b_tz_up);
	if (b_tz_up.value) {
		b_tz_up.value = false;
		new_tz += 1;
	}
	b_tz_down = handleButton(170,26,PSTR(""),'-', b_tz_down);
	if (b_tz_down.value) {
		b_tz_down.value = false;
		new_tz -= 1;
	}
	b_dst = handleButton(220,26,PSTR("DST"),0, b_dst);
	b_led_up = handleButton(120,68,PSTR(""),'+', b_led_up);
	if (b_led_up.value) {
		b_led_up.value = false;
		if (OCR0B > 245)
			OCR0B = 255;
		else
			OCR0B += 10;
	}
	b_led_down = handleButton(170,68,PSTR(""),'-', b_led_down);
	if (b_led_down.value) {
		b_led_down.value = false;
		if (OCR0B >= 10)
			OCR0B -= 10;
		else
			OCR0B = 0;
	}
	b_auto_led = handleButton(220,68,PSTR("Auto"),0, b_auto_led);
	b_dark_mode = handleButton(120,110,PSTR("Enable"),0, b_dark_mode);
	b_auto_mode = handleButton(220,110,PSTR("Auto"),0, b_auto_mode);
	if (b_auto_mode.value)
		b_dark_mode.value = !is_day;
	b_names = handleButton(10,152,PSTR("Names"),0, b_names);
	b_calibrate = handleButton(120,152,PSTR("Calibrate"),0, b_calibrate);
	b_ok = handleButton(10,200,PSTR("Ok"),0, b_ok);
	b_cancel = handleButton(80,200,PSTR("Cancel"),0, b_cancel);
...
static void updateCalibrate(void) {
	...
	ili9341_setTextSize(2);
	b_reset = handleButton(200,136,PSTR("Reset"),0, b_reset);
	b_done = handleButton(60,136,PSTR("Done"),0, b_done);
	ili9341_setTextSize(1);
	if (b_reset.value) {
		b_reset.value = false;
		// reset bounds
		ts_xMax = ts_xMin = ts_yMax = ts_yMin = 0;
		drawCalibrate();
	}
	if (b_done.value) {
		b_done.value = false;
...
// update keyboard screen
static void updateKeyboard(void) {
	static button_t b_key[49], b_up, b_down, b_ok;
	static uint8_t j=0;
	uint8_t i=0, c;
	uint16_t x=2, y=65;

	// draw remote name with cursor
	ili9341_setCursor(10,32);
	drawInt(j);
	ili9341_puts_p(PSTR(": "));
	ili9341_puts(remote[j].name);
	if (action.blink) ili9341_write('_');
	ili9341_clearTextArea(90);
	// draw and handle buttons
	ili9341_setFont(cp437font8x8);
	ili9341_setTextSize(2);
	b_up = handleButton(98, 21, PSTR(""), 24, b_up);
	if (b_up.value) {
		b_up.value = false;
		if (j) j--;
	}
	b_down = handleButton(98, 43, PSTR(""), 25, b_down);
	if (b_down.value) {
		b_down.value = false;
		if (j<SENSOR_COUNT-1) j++;
	}
	uint8_t k = strlen(remote[j].name);
	while ((c = pgm_read_byte(&keys[i]))) {
		if (c == 'a') x += 16;
		if (b_key[39].value) c = toupper(c); // shift
		b_key[i] = handleButton(x, y, PSTR(""), c, b_key[i]);
		if (b_key[i].value && i != 39) {
			b_key[i].value = false;
			b_key[39].value = false;
			if (c == 27)
...
int main(void) {
	...
	while (1) {
		...
		if (view) {
			ili9341_setCursor(193,224);
			drawInt(millis - start);
			ili9341_puts_p(PSTR("ms"));
			ili9341_clearTextArea(240);
		}
